...
const char *DallasSensor::sensorClass(void)
{
    return "DALLAS";
}

const char *DallasSensor::sensorType(void)
{
    return DallasSensor::sensorType(_address);
}

const char *DallasSensor::copyAddressString(void)
{
    char *string;
    char *cursor;
    unsigned char ii;
...
#if defined(__SAM3X8E__) || defined(SPARK)
    sprintf(buffer, "%.2f", this->celsius());
#else
    dtostrf(this->celsius(), 1, 2, buffer);
#endif
    serial->print(buffer);
    return true;
}

void DallasSensor::sendConvertCommand(void)
{
    _oneWire->reset();
...
void DallasSensor::sendReadCommand(void)
{
      byte ii;
      byte data[12];
...
boolean DallasSensor::loop(void)
{
    switch(_state) {
